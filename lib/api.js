...
class ApiClient {
  ...
  async getInbox(params = {}) {
    ...
    return this.get(`/messages/inbox${query ? `?${query}` : ""}`);
  }

  async getSentMessages(params = {}) {
    const query = new URLSearchParams(params).toString();
    return this.get(`/messages/sent${query ? `?${query}` : ""}`);
  }
  ...
  async replyToMessage(messageId, data) {
    return this.post(`/messages/${messageId}/reply`, data);
  }
  ...
  async markMessageAsUnread(id) {
    return this.post(`/messages/${id}/unread`, {});
  }
